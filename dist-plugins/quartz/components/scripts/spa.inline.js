...
const micromorph_1 = require("micromorph");
...
const isElement = (target) => target?.nodeType === NODE_TYPE_ELEMENT;
...
let p;
async function _navigate(url, isBack = false) {
    isNavigating = true;
    startLoading();
    p = p || new DOMParser();
    const contents = await (0, util_1.fetchCanonical)(url)
        .then((res) => {
    ...
    (0, path_1.normalizeRelativeURLs)(html, url);
    ...
    document.querySelector(".navigation-progress")?.remove();
    (0, micromorph_1.default)(document.body, html.body);
    ...
    notifyNav((0, path_1.getFullSlug)(window));
