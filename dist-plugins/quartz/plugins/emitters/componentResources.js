...
exports.ComponentResources = void 0;
const crypto_1 = require("crypto");
const path_1 = require("../../util/path");
// @ts-ignore
const spa_inline_1 = require("../../components/scripts/spa.inline");
// @ts-ignore
const popover_inline_1 = require("../../components/scripts/popover.inline");
const base_scss_1 = require("../../styles/base.scss");
const custom_scss_1 = require("../../styles/custom.scss");
const popover_scss_1 = require("../../components/styles/popover.scss");
...
function getComponentResources(ctx) {
    ...
    for (const component of registry_1.componentRegistry.getAllComponents()) {
        allComponents.add(component);
    }
    ...
    for (const component of allComponents) {
        ...
        for (const c of (0, resources_1.normalizeResource)(css))
            componentResources.css.add(c);
        for (const b of (0, resources_1.normalizeResource)(beforeDOMLoaded))
            componentResources.beforeDOMLoaded.add(b);
        for (const a of (0, resources_1.normalizeResource)(afterDOMLoaded))
...
async function joinScripts(scripts) {
    ...
    // minify with esbuild
    const res = await (0, esbuild_1.transform)(script, {
        minify: true,
    });
    return res.code;
}
function addGlobalPageResources(ctx, componentResources) {
    ...
    if (cfg.enablePopovers) {
        componentResources.afterDOMLoaded.push(popover_inline_1.default);
        componentResources.css.push(popover_scss_1.default);
    }
...
('script')[0];
    ...
    if (cfg.enableSPA) {
        componentResources.afterDOMLoaded.push(spa_inline_1.default);
    }
    else {
        componentResources.afterDOMLoaded.push(`
    ...
    return {
        ...
        async *emit(ctx, _content, resources) {
            ...
            if (cfg.theme.fontOrigin === "local") {
                ...
                const response = await fetch((0, theme_1.googleFontHref)(theme));
                googleFontsStyleSheet = await response.text();
                if (theme.typography.title) {
                    const title = ctx.cfg.configuration.pageTitle;
                    const response = await fetch((0, theme_1.googleFontSubsetHref)(theme, title));
                    googleFontsStyleSheet += `\n${await response.text()}`;
                }
                if (!cfg.baseUrl) {
                    throw new Error("baseUrl must be defined when using Google Fonts without cfg.theme.cdnCaching");
                }
                const { processedStylesheet, fontFiles } = await (0, theme_1.processGoogleFonts)(googleFontsStyleSheet, cfg.baseUrl);
                ...
                for (const fontFile of fontFiles) {
                    ...
                    yield (0, helpers_1.write)({
                        ctx,
                        slug: (0, path_1.joinSegments)("static", "fonts", fontFile.filename),
            ...
            const globalCss = componentResources.css.filter((c) => !componentResources.componentCssStrings.has(c));
            // Core CSS: theme + fonts + global CSS + base styles (no per-component CSS)
            const quartzBase = (0, theme_1.joinStyles)(ctx.cfg.configuration.theme, googleFontsStyleSheet, ...globalCss, base_scss_1.default);
            const stylesheet = `@layer quartz-base {\n${quartzBase}\n}\n${custom_scss_1.default}`;
            ...
            const cssContent = (0, lightningcss_1.transform)({
                filename: "index.css",
                code: Buffer.from(stylesheet),
                minify: true,
                targets: lightningTargets,
                include: lightningcss_1.Features.MediaQueries,
            }).code.toString();
            const cssStringToFilename = new Map();
            for (const cssString of componentResources.componentCssStrings) {
                if (cssStringToFilename.has(cssString))
                    continue;
                const wrapped = `@layer quartz-base {\n${cssString}\n}`;
                const minified = (0, lightningcss_1.transform)({
            ...
            for (const cssResource of resources.css) {
                if (!(cssResource.inline ?? false))
                    continue;
                let output;
                try {
                    output = (0, lightningcss_1.transform)({
            ...
            for (const jsResource of resources.js) {
                if (jsResource.contentType !== "inline")
                    continue;
...
exports.ComponentResources = ComponentResources;
